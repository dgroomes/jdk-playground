...
abstract public class ExtraModuleInfoTransform implements TransformAction<ExtraModuleInfoTransform.Parameter> {
    ...
    private static byte[] addModuleInfo(ModuleInfo moduleInfo) {
        ...
        for (String packageName : moduleInfo.getExports()) {
            // Why do the dot characters need to be replaced? And why don't the dot characters need to be replaced on
            // the calls to "visitRequire"?
        ...
        for (Provides provides : moduleInfo.getProvides()) {
            moduleVisitor.visitProvide(provides.service().replace('.', '/'), provides.provider().replace('.', '/'));
        }
